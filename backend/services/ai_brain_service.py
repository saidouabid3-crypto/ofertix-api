...
class AIBrainService:
    """Authenticated Deal Brain: deterministic rules with an optional, now
    ...
    def history(self, current_user: dict, limit: int = 30):
        return {
            "items": ai_brain_repository.history(
    ...
    def _rules_analysis(self, payload) -> dict:
        ...
        score = DealScoreService.calculate_score(
            current_price=current or 0, old_price=old
        )
        ...
        if payload.specs:
            reasons.append(
                "Specs detectadas: se preparó explicación simple para compra informada."
            )
    ...
    async def _llm_summary(self, payload, base: dict) -> str:
        ...
        # The summary language follows the active request locale; the structured
        # ai_service path replies in the user's language automatically.
    ...
    def _alternatives(self, payload) -> list[str]:
        ...
        if not title:
            return [
                "Buscar alternativa reacondicionada",
                "Comparar con marca equivalente",
                "Esperar una rebaja verificada",
            ]
        return [
    ...
    def _explain_specs(self, specs: str) -> list[str]:
        ...
        for part in parts[:5]:
            ...
            if "ram" in lower:
                ...
                explanations.append(
                    f"{part}: revisa si el almacenamiento te llega para 2-3 años."
                )
            elif "hz" in lower:
                explanations.append(
                    f"{part}: pantalla más fluida, útil para gaming y scroll."
                )
            else:
                explanations.append(
                    f"{part}: compáralo con tu uso real antes de pagar extra."
                )
        return explanations
